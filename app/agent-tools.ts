...
function findConcepts(atlas:Atlas,query:string){
 const q=query.toLowerCase().trim();
 return atlas.concepts.filter(c=>c.name.toLowerCase().includes(q)||c.id.toLowerCase().includes(q)).sort((a,b)=>{
  const an=a.name.toLowerCase(),bn=b.name.toLowerCase();
  const as=an===q?0:an.startsWith(q)?1:2,bs=bn===q?0:bn.startsWith(q)?1:2;
  return as-bs||a.name.length-b.name.length;
 }).slice(0,30);
}
export function atlasTools(atlas:Atlas,inspect:(concept:Concept)=>void):Tool[]{return [
 {name:'find_anatomy',description:'Find anatomical structures by name or source atlas identifier in this atlas.',inputSchema:{type:'object',properties:{query:{type:'string',minLength:1}},required:['query'],additionalProperties:false},annotations:{readOnlyHint:true},execute(input){const data=record(input);if(typeof data.query!=='string'||!data.query.trim())throw new Error('A nonempty query is required.');return findConcepts(atlas,data.query).map(c=>({id:c.id,name:c.name,pieces:c.elements.length}));}},
