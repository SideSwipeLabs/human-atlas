...
import {Activity,ArrowUpFromLine,ArrowUpRight,Copy,Eye,EyeOff,Focus,Info,Layers3,Link2,Move,Pause,PersonStanding,RotateCcw,RotateCw,Scan,Search,Tag,Undo2,X} from 'lucide-react';
...
export default function Home(){
 ...
 },[drawer,searchOpen,about,details,state.isolate,state.dissect,state.selected.length]);

 const fact=chosen&&selected?factFor(chosen.name,displaySystem(selected)):null;
 const hint=state.dissect?'Dissect: drag a named part to pull it out. Drag empty stage to rotate.':state.isolate?'Only the selection is shown':!poseIsRest(state.pose)?'Posed for study':state.xray?'Muscle wall faded':pulled?`${pulled} pulled out`:'Drag to rotate. Tap a structure for its name.';

 return <main className={`studio ${details&&selectedParts.length>0?'has-inspector':''} ${state.dissect?'is-dissect':''} ${searchOpen?'searching':''}`}>
  {atlas&&<AnatomyScene atlas={atlas} state={{...state,inspectorOpen:details&&selectedParts.length>0}} onSelect={choosePart} onProgress={n=>{setProgress(n);if(n===100)setError('');}} onError={setError} onPulled={setPulled}/>}
  ...
  {drawer==='layers'&&<section className="drawer" aria-label="Dissection layers">
   ...
   <div className="chip-row wrap">{REGIONS.map(r=><button type="button" key={r.id} aria-pressed={state.region===r.id} className={state.region===r.id?'on':''} onClick={()=>setState(s=>({...s,region:r.id,reset:r.id==='full'?s.reset+1:s.reset,rotate:false}))}>{r.name}</button>)}</div>
