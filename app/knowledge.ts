...
const K:Record<string,Fact>={
 ...
 'subscapularis':{latin:'Musculus subscapularis',kind:'Rotator cuff',summary:'The cuff muscle on the costal surface of the scapula, inserting on the lesser tubercle of the humerus. It rides with scapular upward rotation, not with glenohumeral spin.',function:'Internally rotates the arm and keeps the humeral head seated in the glenoid.',source:'FMA 13413.'},
 'infraspinatus':{latin:'Musculus infraspinatus',kind:'Rotator cuff',summary:'From the infraspinous fossa to the greater tubercle. The belly rides with scapular upward rotation; the insertion follows the humerus.',function:'Externally rotates the arm.',source:'FMA 32546.'},
 'pectoralis major':{latin:'Musculus pectoralis major',kind:'Skeletal muscle',summary:'The fan of the anterior chest, from clavicle and sternum to the lateral lip of the bicipital groove.',function:'Adducts, flexes, and internally rotates the arm. When the arm is raised, the sternal origin stays on the chest and the insertion follows the humerus so the axilla opens.',source:'FMA 9628.'},
...
export function factFor(name:string,system:SystemId):Fact&{side:string}{
 ...
 if(/\bdeltoid\b/.test(n)&&!/branch|artery|vein/.test(n))return {...K['deltoid'],side};
